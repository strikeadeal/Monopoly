...
import { auctionsEnabled, type GameState, type TokenId } from '@monopoly/game';
...
export function Lobby({ state, playerId, send, onLeave, leaving, leaveError, error, clearError }: { state: GameState; playerId: string; send: (command: Record<string, unknown> & { type: string }) => void; onLeave: () => void | Promise<void>; leaving: boolean; leaveError: string | null; error?: string | null; clearError?: () => void }) {
  ...
  return <main className="lobby-shell">
    ...
    {isHost ? <section className="lobby-options"><h2>Table options</h2><button type="button" className="ready-button" aria-pressed={auctionsOn} onClick={() => send({ type: 'SET_AUCTIONS', enabled: !auctionsOn })}>Auctions {auctionsOn ? 'on' : 'off'}</button><p className="rule-note">{auctionsOn ? 'Declined and returned deeds go to auction.' : 'Declined and returned deeds stay with the Bank.'}</p></section> : null}
