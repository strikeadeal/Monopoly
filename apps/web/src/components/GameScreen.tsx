...
function PlayerBalances({ state, playerId }: { state: GameState; playerId: string }) {
  return <section className="player-balances" aria-label="Player balances">{state.turnOrder.map((id) => {
    const player = state.players.find((candidate) => candidate.id === id)!;
    const labels = [player.name];
    if (player.id === playerId) labels.push('you');
    if (player.id === state.currentPlayerId) labels.push('current player');
    if (player.bankrupt) labels.push('bankrupt');
    labels.push(money.format(player.cash));
    return <article key={player.id} className={`balance-chip${player.id === playerId ? ' is-me' : ''}${player.id === state.currentPlayerId ? ' is-current' : ''}${player.bankrupt ? ' is-bankrupt' : ''}`} aria-label={labels.join(', ')}><span className="balance-token"><TokenIcon token={player.token} size={16} /></span><span><strong>{player.name}</strong><small>{money.format(player.cash)}</small></span></article>;
  })}</section>;
}
...
export function GameScreen({ state, playerId, status, error, send, clearError }: { state: GameState; playerId: string; status: string; error: string | null; send: Sender; clearError: () => void }) {
  ...
  return <main className="game-shell"><span className="sr-only" aria-live="polite">{movement.announcement}</span><header className="status-rail"><div><span className={`status-dot ${status}`} />{status === 'online' ? 'Live' : 'Reconnecting'}</div><strong>{current?.name}{current?.id === playerId ? ' · your turn' : ' · playing'}</strong><span>{timer ?? `Round ${state.round}`}</span>{state.hostPlayerId === playerId && state.phase.type !== 'finished' ? <button className="table-control" aria-label={state.phase.type === 'paused' ? 'Resume game' : 'Pause game'} onClick={() => send({ type: state.phase.type === 'paused' ? 'RESUME' : 'PAUSE', now: Date.now() })}>{state.phase.type === 'paused' ? '▶' : 'Ⅱ'}</button> : null}</header><PlayerBalances state={state} playerId={playerId} />{error ? <div className="toast" role="alert"><span>{error}</span><button onClick={clearError}>×</button></div> : null}{tab === 'game' ? <><Board state={state} selectedIndex={selected} onSelect={setSelected} displayPositions={movement.displayPositions} movingPlayerId={movement.movingPlayerId} tokenMotion={movement.tokenMotion} />{movementOverridesActions && movingPlayer ? <MovementTurnCard state={state} playerName={movingPlayer.name} rolling={rolling} /> : <TurnActions state={state} playerId={playerId} send={send} rolling={rolling} />}</> : null}{tab === 'assets' ? <Assets state={state} playerId={playerId} send={send} /> : null}{tab === 'trade' ? <Trade state={state} playerId={playerId} send={send} /> : null}{tab === 'activity' ? <section className="tab-panel"><h2>Activity</h2><ol className="activity-list">{state.activities.map((entry) => <li key={entry.id} className={entry.tone}><time>{new Date(entry.at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</time><span>{entry.text}</span></li>)}</ol></section> : null}<nav className="bottom-nav" aria-label="Game sections">{(['game', 'assets', 'trade', 'activity'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'assets' ? 'My assets' : item[0]!.toUpperCase() + item.slice(1)}</button>)}</nav>{selected !== null ? <PropertySheet state={state} index={selected} playerId={playerId} send={send} onClose={() => setSelected(null)} /> : null}{showCard ? <CardReveal state={state} draw={lastCard} onClose={closeCard} /> : null}{status !== 'online' ? <div className="reconnect-sheet"><div className="spinner" /><h2>Reconnecting to the table</h2><p>Your last confirmed board is safe. Controls will return after a fresh server snapshot.</p></div> : null}</main>;
}
