...
function TurnActions({ state, playerId, send, rolling }: { state: GameState; playerId: string; send: Sender; rolling: boolean }) {
  ...
  if (phase.type === 'purchase') { const space = BOARD[phase.spaceIndex] as Extract<(typeof BOARD)[number], { price: number }>; return <div className="turn-card purchase-card"><span className="eyeline">AVAILABLE</span>{state.lastRoll ? <div className="dice-line"><DiceRoll roll={state.lastRoll} rolling={rolling} /></div> : null}<h2>{space.name}</h2><p>Buy for {money.format(space.price)} or send it to auction.</p><div className="action-row"><button className="primary-button" onClick={() => send({ type: 'BUY_PROPERTY' })}>Buy</button><button onClick={() => send({ type: 'DECLINE_PROPERTY', now: Date.now() })}>Auction</button></div></div>; }
...
function Trade({ state, playerId, send }: { state: GameState; playerId: string; send: Sender }) {
  ...
  const deedCount = (count: number) => `${count} ${count === 1 ? 'deed' : 'deeds'}`;
  if (pending?.toPlayerId === playerId) return <section className="tab-panel trade-offer"><span className="eyeline">TRADE OFFER</span><h2>{state.players.find((player) => player.id === pending.fromPlayerId)?.name} proposes a deal</h2><p>They offer {money.format(pending.offeredCash)} and {deedCount(pending.offeredProperties.length)} for {money.format(pending.requestedCash)} and {deedCount(pending.requestedProperties.length)}.</p><div className="action-row"><button className="primary-button" onClick={() => send({ type: 'RESPOND_TRADE', accept: true })}>Accept</button><button onClick={() => send({ type: 'RESPOND_TRADE', accept: false })}>Decline</button></div></section>;
